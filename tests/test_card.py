...
def test_card_dataframe_functionality():
    ...
    df = pd.DataFrame({
        ...
        '创建时间': [datetime(2024, 1, 1), datetime(2024, 1, 2), datetime(2024, 1, 3)]
    })
...
def test_card_dataframe_with_bot():
    ...
    # 生成表格卡片
    table_element = CardElementGenerator.table_card(
        df,
        page_size=4,
        row_height='auto',
    ...
    response = bot.send_card(
        content=[
            CardElementGenerator.markdown("## 产品销售数据报表"),
            table_element
        ],
        title="数据表格测试",
        echo=True
